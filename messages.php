...
$OwnerID       = isset($_GET['id'])            ? $_GET['id']              : '';
$MessCategory  = isset($_GET['messcat'])       ? $_GET['messcat']         : '';
$MessPageMode  = isset($_GET['mode'])          ?  (string)$_GET['mode']   : '';
$DeleteWhat  = isset($_GET['deletemessages'])  ?  $_GET['deletemessages'] : '';
if ($DeleteWhat != '') {
	$MessPageMode = "delete";
}
	...
	switch ($MessPageMode) {
		case 'write':
			...
			if ($_POST) {
				...
				if (!$_POST["subject"]) {
					$error++;
					$parse["error_label"] = $lang['mess_no_subject'];
				}
				if (!$_POST["text"]) {
					$error++;
					$parse["error_label"] = $lang['mess_no_text'];
				}
				if ($error == 0) {
...
//					$_POST['text'] = str_replace('\r\n', '<br />', $_POST['text']);
			...
			$parse["message_sent"] = $messageSent;
			$parse['message_sent_label']  = $lang['mess_sended'];
			$parse['error_occured'] = $error > 0;
			...
			$page                 = $MustacheEngine->render(gettemplate('messages_pm_form'), $parse);
