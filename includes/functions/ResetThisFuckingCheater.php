...
function ResetThisFuckingCheater ( $UserID ) {
	...
	if($TheUser['authlevel'] == 0){
		...
		if ($UserPlanet['name'] != "") {
			...
			//Warning Message
			$sender = 1;
			$Subject = $lang['user_reset_attempt'];
			$message = $lang['user_reset_cheater'];
			SendSimpleMessage($NewUser['id'], $sender, time(), 1, "System", $Subject, $message);

			//Warn Admins
			$universeOperators = doquery("SELECT `id` FROM {{table}} WHERE authlevel != 0", "users");
			$Subject = $lang['user_reset_attempt'];
			$message = $lang['user_reset_attempt_adm'].$NewUser['id'];
			while($operator = mysqli_fetch_array($universeOperators)){
				
				SendSimpleMessage($operator['id'], $sender, time(), 1, "System", $Subject, $message);
			}
