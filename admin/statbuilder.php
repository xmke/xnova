...
if (strtolower(substr(PHP_SAPI, 0, 3)) == 'cli' || in_array($user['authlevel'], array(LEVEL_ADMIN, LEVEL_OPERATOR, LEVEL_MODERATOR))) {
	...
	while ($CurUser = mysqli_fetch_assoc($GameUsers)) {
		...
		if(isset($OldStatRecord)){
			$OldStatRecordArray = mysqli_fetch_assoc($OldStatRecord);
			if(isset($OldStatRecordArray)){
				$OldTotalRank = $OldStatRecordArray['total_rank'];
				$OldTechRank  = $OldStatRecordArray['tech_rank'];
				$OldBuildRank = $OldStatRecordArray['build_rank'];
				$OldDefsRank  = $OldStatRecordArray['defs_rank'];
				$OldFleetRank = $OldStatRecordArray['fleet_rank'];
